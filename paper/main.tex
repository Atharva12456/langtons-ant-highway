...
\begin{proof}
Eliminating the four $F_q$ from \eqref{eq:pot} around a unit plaquette gives the
four-corner condition \eqref{eq:cocycle}. For the second condition, combine the four
relations at a single cell $z$: the $\turnR$-at-$S$ and $\turnL$-at-$N$ relations both
express $F_W(z+u_W)$, forcing $F_S(z)=F_N(z)+2w(z)$, while the $\turnL$-at-$S$ and
$\turnR$-at-$N$ relations force $F_S(z)=F_N(z)-2w(z)$; hence $4w(z)=0$. \emph{(}Thus
$w(x,y)=(-1)^{x+y}$ over $A=\Z$ satisfies \eqref{eq:cocycle} but admits no such $F_q$,
since $4w\neq0$.\emph{)} Conversely, over $A=\Z/4\Z$ the four-corner condition lets one
build $F_N$ consistently (its increments around every plaquette cancel), and setting
$F_S=F_N-2w$, $F_E,F_W$ by \eqref{eq:pot} is consistent because $4w=0$; the $\F_2$
solution space is the separable weights $\alpha(x)+\beta(y)$.
\end{proof}
...
$\drift$ preserves $\che$, and $w(z+\drift)=w(z)$. Write $\drift=(a,b)$ and move from
$z$ to $z+\drift$ along $a$ diagonal $(1,\operatorname{sgn} b)$-steps followed by the
remaining vertical displacement; each diagonal step contributes
$-\che\cdot(\alpha(x)+\alpha(x{+}1))$ by \eqref{eq:diag} (or its $(1,-1)$ analogue),
and the vertical displacement is even, contributing nothing by the $2$-periodicity of
$F_N$ in $y$. As $x$ runs once through $\Z/|a|\Z$ the diagonal contributions
...
Taking $s,t$ to be the states immediately after an $\turnR$ visit to a cell and
immediately before its next ($\turnL$) visit gives $\eta(s)=\eta(t)$, so the odd-visit
defect between them is Eulerian; a companion analysis \cite{Repo} shows this forces at
least four distinct cells, but we do not use that refinement here.
...
\end{tabular}
\end{center}
(read as one word of $104$ symbols, four rows of $26$)
