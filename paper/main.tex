...
The engines' source, the per-shard records with SHA-256 manifests, the aggregation and
cross-audit scripts, and the exact commands to regenerate every shard accompany this
work \cite{Repo}; the searches were run on a multi-core cluster with one worker per
shard. \emph{(All records are exactly regenerable from the released sources by the
commands given there; readers who wish to reproduce the exclusion need only rerun the
reference engine.)}
...
\newblock \url{https://github.com/Atharva12456/langtons-ant-highway}.
