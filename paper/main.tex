...
that \emph{occur}; proving nonexistence below a period bound is new.
\end{enumerate}
...
\end{equation}
which define $F_N$ on each of the two parity sublattices by integration from a
basepoint. The sum of the four increments around an elementary diamond
$(x,y)\to(x{+}1,y{+}1)\to(x{+}2,y)\to(x{+}1,y{-}1)\to(x,y)$ telescopes, by
\eqref{eq:Fdiag}, to exactly the four-corner sum \eqref{eq:cocycle} at $(x,y{-}1)$ and
$(x{+}1,y{-}1)$; the four-corner condition makes it vanish, so the integration is
path-independent and $F_N$ is well defined. Now set $F_S=F_N-2w$ and define $F_E,F_W$
...
with $L>0$ even---all that the argument below requires. Reversibility and translation
covariance give, for all $n\in\Z$ and phases $j$,
\[
...
$z$ to $z+\drift$ by $|a|$ diagonal steps of vector $(\varepsilon,\sigma)$ (using
\eqref{eq:diag} for $\varepsilon=+1$ and its reverse for $\varepsilon=-1$), followed
by the vertical displacement $(0,\,b-\sigma|a|)$, which is even because $a+b$ is even.
Each diagonal step contributes $-\che\cdot(\alpha(x)+\alpha(x{+}1))$ by \eqref{eq:diag}
(or its $(1,-1)$ analogue), while the vertical part contributes nothing by the
$2$-periodicity of $F_N$ in $y$. As $x$ runs once through $\Z/|a|\Z$ the diagonal
contributions
telescope to $-2\che(z)\sum_{r}\alpha(r)$, giving the monodromy
\begin{equation}\label{eq:monodromy}
F_N(z+\drift)-F_N(z)\ \equiv\ 2\sum_{r\in\Z/|a|\Z}\alpha(r)\pmod4
\end{equation}
(using $-2\equiv2$). The three other headings satisfy the same relation: $F_S=F_N-2w$
...
heading-reset nonzero-drift words through length $18$, and the strand identity
\eqref{eq:res-a} together with the bound \eqref{eq:P16} was confirmed on the standard
highway, its powers, and its dihedral images---the smallest realisable nonzero-drift
traces, since none of period at most $18$ exists.
