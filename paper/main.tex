...
$\drift=\gcd(a,b)\cdot(a',b')$ with $\gcd(a',b')=1$; then the pair
\[
\bigl(\,b'x-a'y,\ (a'x+b'y)\bmod\gcd(a,b)\,\bigr)
\]
is a complete invariant of the coset of $(x,y)$, the transverse coordinate $b'x-a'y$
together with a torsion residue along the primitive drift direction. (Reducing a
single coordinate modulo a coordinate of $\drift$ is \emph{not} a complete invariant
when $\gcd(a,b)>1$; e.g.\ for $\drift=(2,2)$ the cells $(0,0)$ and $(0,2)$ agree in
$x\bmod 2$ yet lie in different cosets.) This is the checker used in all computations
...
Let $A$ be an abelian group and $w:\Z^2\to A$. There exist heading corrections
$F_q:\Z^2\to A$ making $\sum_{z\in B}w(z)+F_q(p)$ conserved if and only if $w$
satisfies the four-corner condition
...
\end{equation}
For $A=\F_2$ the solutions of \eqref{eq:cocycle} are exactly $w(x,y)=\alpha(x)+\beta(y)$.
\end{proposition}
...
$f(n)=D(c+n\drift)\in\Z$, a finitely supported function $\Z\to\Z$. Any such $f$ has a
unique decomposition
\[
f=\Bigl(\sum_{n}f(n)\Bigr)\delta_{0}+(T-1)a,\qquad a(n)=-\!\!\sum_{m>n}\! f(m),
\]
where $(Ta)(n)=a(n-1)$: indeed $(T-1)a$ has zero total and telescopes to
$f-(\sum f)\delta_0$, and $a$ is finitely supported because $f$ is. Summing this
over all classes gives \eqref{eq:decomp} with $A$ the (finite) sum of the class
antiderivatives and $S=\sum_{\text{classes}}(\sum_n f)\,\delta_{c}$. It remains to
...
If $\growth=0$ then $S=0$, so $D=(T_\drift-1)A$. In this case the black
configuration is, up to a finite stationary set, a single $\drift$-translate of the
finite widget $A$; because the trace is periodic with nonzero drift, every cell is
visited only finitely often, so after finitely many periods the ant never revisits
the stationary set. Deleting that set changes no future turn, and the remaining state
is a clean finite translator with drift $\drift$.
\end{corollary}
...
survivors. Because both engines traverse the same tree and reach the same leaves,
their node and leaf counts are \emph{identical} by construction; only the number of
exact-criterion evaluations differs (far fewer in the residue engine). The two
cross-checks are thus complementary: the reference engine certifies zero hits without
assuming Theorem~\ref{thm:residue}, while agreement of all per-rank node, leaf, and
prune counters certifies that the residue pruning discarded no realisable word. For
period $48$ both engines reported $8{,}677{,}026{,}370$ nodes and $451{,}962{,}870$
leaves over the $2^{16}$ prefix ranks; the residue engine evaluated the exact
criterion on $12{,}257{,}208$ survivors after rejecting $439{,}705{,}662$ leaves.
...
cross-audit scripts, and the exact commands to regenerate every shard are provided in
the repository \cite{Repo}, which is archived at a fixed commit; the searches were run
on a multi-core cluster with one worker per shard.
