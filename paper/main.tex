...
theorem} (and, building on Traldi's interlacement theory, a touch-graph rank
computation for the periodic trace). \emph{(5) A two-engine, rank-audited exhaustive
computation} excluding every finite-support periodic highway of nonzero drift and
period at most $48$, so that any nonstandard periodic highway must have even period
at least $50$. We also
...
nonzero drift and period at most $48$. Existing computational work enumerates highways
that \emph{occur}; we are not aware of a prior exhaustive nonexistence result below a
fixed period bound.
\end{enumerate}
...
\begin{tikzpicture}[scale=0.95,line width=0.9pt]
  ...
  \begin{scope}
    ...
    \node at (0,-1.2) {\footnotesize crossing at $z$};
  \end{scope}
  % white smoothing: vertical arcs reconnect left/left, right/right (e_z absent)
  \begin{scope}[xshift=3.6cm]
    \draw (-0.8,-0.8) .. controls (-0.15,-0.35) and (-0.15,0.35) .. (-0.8,0.8);
    \draw (0.8,-0.8)  .. controls (0.15,-0.35)  and (0.15,0.35)  .. (0.8,0.8);
    \node at (0,-1.2) {\footnotesize white: $e_z\notin H_B$};
  \end{scope}
  % black smoothing: horizontal arcs; the Tait edge e_z is present (dashed)
  \begin{scope}[xshift=7.2cm]
    \draw (-0.8,-0.8) .. controls (-0.35,-0.15) and (0.35,-0.15) .. (0.8,-0.8);
    \draw (-0.8,0.8)  .. controls (-0.35,0.15)  and (0.35,0.15)  .. (0.8,0.8);
    \draw[dashed] (0,-0.42)--(0,0.42);
    \node at (0,-1.2) {\footnotesize black: $e_z\in H_B$};
  \end{scope}
\end{tikzpicture}
\caption{The two smoothings of the frozen crossing at an ant cell $z$. Colour selects
which of the two ways the four incident frozen arcs reconnect; the black smoothing is
the occupied Tait edge $e_z$ (dashed).}
\label{fig:smoothing}
\end{figure}
...
well defined. Now set $F_S=F_N-2w$ and define $F_E,F_W$
from $F_N$ by \eqref{eq:pot}; the two ways of reaching $F_S$ agree precisely because
$4w=0$, so all four $F_q$ are consistent. Over $\F_2$ the solution space of
\eqref{eq:cocycle} is the separable weights $\alpha(x)+\beta(y)$.
\end{proof}
...
reference engine applied the exact criterion of Theorem~\ref{thm:criterion} to every
leaf, and the residue-pruned engine applied it only to the survivors of
Theorem~\ref{thm:residue}. Both returned zero certificates, and their per-rank node,
leaf, and common legality-pruning counters agree; the reference engine's exhaustive
outcome is the certificate of nonexistence, and the residue engine is a cross-check.
...
combinations of $(E,V,k,\beta)$ considered in Remark~\ref{rem:nomonotone} is monotone;
...
Two independent engines are run for the period $42$--$48$ certifications. The
\emph{reference engine} applies the exact criterion of Theorem~\ref{thm:criterion} to
every positive-growth leaf and makes \emph{no} use of Theorem~\ref{thm:residue}; its
zero-hit outcome is therefore unconditional on the residue theorem. The
\emph{residue engine} explores the identical prefix tree but first rejects each leaf
violating Theorem~\ref{thm:residue}, applying the exact criterion only to the
survivors. Because both engines enumerate the same tree and reach the same leaves,
their node counts, leaf counts, and common legality-pruning counts are identical per
shard by construction; they differ only in the residue-prune and exact-evaluation
counts, which the residue engine reports and the reference engine does not use. The
exclusion itself rests on the reference engine: it applies
Theorem~\ref{thm:criterion} to \emph{every} leaf and finds no realisable word,
independently of Theorem~\ref{thm:residue}. The residue engine is an implementation
cross-check: its own zero-hit result, together with the identical node/leaf coverage,
confirms that the residue pruning discarded no realisable word. For period $48$ both
engines reported $8{,}677{,}026{,}370$ nodes and $451{,}962{,}870$ leaves over the
$2^{16}$ prefix ranks; the residue engine evaluated the exact criterion on
$12{,}257{,}208$ survivors after rejecting $439{,}705{,}662$ leaves.
...
available in the accompanying repository \cite{Repo} at the tagged release
\texttt{v1.0}; the searches were run on a multi-core cluster with one worker per shard.
\emph{(All records are exactly regenerable from the released sources by the commands
given there; readers who wish to reproduce the exclusion need only rerun the reference
engine.)}
